...
public class FinancialService {
        ...
        public List<Payroll> getAllPayrolls(java.time.LocalDate month, Long plantationId) {
                if (plantationId != null) {
                        online.teaplanter.backend.entity.Plantation plantation = plantationRepository
        ...
        public online.teaplanter.backend.dto.PayrollPreviewDTO getPayrollPreview(Long workerId,
                ...
                List<online.teaplanter.backend.entity.Harvest> harvests = harvestRepository
                                .findByWorkerAndHarvestDateBetween(worker, start, end);
                double harvestEarnings = harvests.stream()
                                .mapToDouble(h -> h.getCalculatedPay() != null ? h.getCalculatedPay() : 0.0)
                                .sum();

                List<online.teaplanter.backend.entity.Task> tasks = taskRepository
                                .findByAssignedWorkerAndCompletedAtBetween(worker, start.atStartOfDay(),
                                                end.atTime(23, 59, 59));
                double taskEarnings = calculateTaskEarnings(tasks);

                return online.teaplanter.backend.dto.PayrollPreviewDTO.builder()
        ...
        public Double getTotalInventoryExpenses(java.time.LocalDate month, Long plantationId) {
                ...
                java.util.stream.Stream<online.teaplanter.backend.entity.StockEntry> entries = stockEntryRepository
        ...
        private double calculateTaskEarnings(List<online.teaplanter.backend.entity.Task> tasks) {
                return tasks.stream()
                                .mapToDouble(t -> {
